  ...
  <body>
    ...
    <script>
      ...
      function startSoftAudio() {
        ...
        music.type = "sine";
        music.frequency.setValueAtTime(220, context.currentTime);
        musicGain.gain.setValueAtTime(0.025, context.currentTime);
        music.connect(musicGain);
        musicGain.connect(context.destination);
        music.start();
        for (let index = 0; index < stages.length; index += 1) {
          const when = context.currentTime + stages[index].start + 0.12;
          createDing(context, when, -0.85, 0.85);
        }
      }
      ...
      function startInteractivePlayback() {
        startSoftAudio();
        startSpeechSequence();
      }

      window.addEventListener("pointerdown", startInteractivePlayback, { once: true });
      window.addEventListener("keydown", startInteractivePlayback, { once: true });
      window.addEventListener("hf-play", startInteractivePlayback);
    </script>
  </body>
</html>
"""
